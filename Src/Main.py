...
def main():
    ...
    if choice == "1":
        ...
        evt_stime = input(" Start Time (HH:MM, optional): ").strip()
        evt_etime = input(" End Time (HH:MM, optional): ").strip()
        ...
        try:
            ...
            print(f"\n  Invalid input: {e}\n")
            return
        if System(events).is_overlapped(evt_stime) and System(events).is_solid(evt_date,evt_stime):
            print(not System(events).is_solid(evt_date,evt_stime))
...
if __name__ == "__main__":
    ...
    while con:
        main()
        con = input("Conitnue? (y/n)").strip().lower() == "y"
